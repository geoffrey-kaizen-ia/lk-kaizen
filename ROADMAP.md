...
- [ ] EN COURS (29/06) — GARDE-FOU ANTI-DOUBLON ICEBREAKER (WF `0yQOYs1Ffiqtj4IX`, branche cron) : bloque l'envoi si une conversation existe deja avec le prospect. 4 nodes inseres avant `Unipile - Envoyer icebreaker1` : A `Unipile - Chats du compte` (GET `/chats` par account_id), B Code `Deja un chat ?` (match `attendee_provider_id` == linkedin_id -> booleen `already_contacted`), C IF `Deja contacte ?`, D Supabase `Update Prospect1` (branche skip : status=in_conversation + processing_status=idle). Node B valide (renvoie `true` sur Anthony qui a un chat). RESTE A FAIRE : cabler C+D, re-tester Anthony (doit partir sur TRUE sans envoi), re-exporter le WF, supprimer la fiche test Anthony (id `3062d60e-85f7-4cb0-9a94-a44ecbecb048`).
